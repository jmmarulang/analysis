...
(mu (f @^-1` (setT `\ 0%R))) when p = 0
*)
Lemma Lnorm_EFin f r : 'N_r%:E[f] = (\int[mu]_x `|f x| `^ r%:E) `^ (r^-1)%:E.
...
- rewrite eqe => r0.
  ...
  by rewrite integral0 poweR0e// invr_neq0.
...
Lemma Lnorm_cst1 r : 'N_r%:E[cst 1] = mu [set: T] `^ (r^-1)%:E.
Proof.
rewrite unlock /Lnorm; under eq_integral do rewrite/= normr1 poweR1.
...
- rewrite lte_fin => r0 /eqP; rewrite -poweR_eq0/=. 
  repeat case: ifPn=>//; last first.
    by rewrite lte_fin invr_lt0; move: r0; case: (ltgtP r 0)%R.
  move=> /eqP h /[!@eqe] /[!@invr_eq0] rneq0.
  have : \int[mu]_x `| `|f x| `^ r%:E | = 0%R.
    under eq_integral.
      move=> x _; rewrite gee0_abs; last by rewrite poweR_ge0. over.
    by apply h.
  have mp : measurable_fun [set: T] (fun x => `|f x| `^ r%:E).
    apply: ( @measurableT_comp _ _ _ _ _ _ (fun x => x `^ r%:E)) => //=.
      exact: (measurableT_comp (measurable_poweR _)).
    exact: measurableT_comp.
  move/(ae_eq_integral_abs mu measurableT mp).
  apply: filterS => x/= /[apply].
  move=> /eqP; rewrite -poweR_eq0 //=; repeat case: ifPn=>//; last first.
...
Lemma measurableT_comp_poweR f y :
  measurable_fun [set: T] f -> measurable_fun setT (fun x => f x `^ y).
Proof. 
exact /(@measurableT_comp _ _ _ _ _ _ (@poweR R ^~ y)) /measurable_poweR. Qed.

Lemma integrable_poweR f r : (0 < r)%R ->
...
move=> /eqP/(@eqy_poweR _ _ (r^-1)%:E); rewrite lte_fin invr_gt0 => /(_ p0) <-.
rewrite unlock; apply/eqP; congr (_ `^ _).
by apply/eq_integral=> t _ ;rewrite [RHS]gee0_abs// poweR_ge0.
Qed.

Lemma le_Lnorm f g r : 
  measurable_fun [set: T] f ->
  measurable_fun [set: T] g ->
  (0 <= r)%R ->
  {in [set: T],  forall x : T, `|f x| <= `|g x| } ->
  'N_r%:E[f] <= 'N_r%:E[g].
Proof.
move=> ? ? ? H.
rewrite unlock; apply ge0_lee_poweR; first by rewrite lee_fin invr_ge0.
- by rewrite in_itv//= leey integral_ge0; last by move=> x _; rewrite poweR_ge0.
- by rewrite in_itv//= leey integral_ge0; last by move=> x _; rewrite poweR_ge0.
apply ge0_le_integral=>//=; first by move=> x _; rewrite poweR_ge0.
- by apply/measurableT_comp_poweR/measurableT_comp.
- by apply/measurableT_comp_poweR/measurableT_comp.
move=> x _; apply /ge0_lee_poweR=>//.
- by rewrite in_itv//= abse_ge0 leey.
- by rewrite in_itv//= abse_ge0 leey.
by apply H; rewrite inE.
Qed.
...
Variable mu : {measure set T -> \bar R}.
Local Open Scope ereal_scope.
Implicit Types (p q : R) (y z : \bar R) (f g : T -> \bar R) (h i : T -> R).
...
rewrite (ae_eq_integral (cst 0)) => [|//||//|]; first by rewrite integral0.
  - repeat apply: measurableT_comp=> //; exact: emeasurable_funM.
  - apply: filterS (Lnorm_eq0_eq0 mf p0 f0) => x /(_ I) + _.
    by rewrite /= abseM=> ->; rewrite abse0 mul0e.
Qed.

Let normalized p f x := (`|f x| / 'N_p%:E[f]).

Let normalized_ge0 p f x : (0 <= normalized p f x).
Proof. by rewrite /normalized mule_ge0// inve_ge0 Lnorm_ge0. Qed.
...
transitivity (\int[mu]_x (`|f x| `^ p%:E / ('N_p%:E[f] `^ p%:E))).
  apply: eq_integral => t _; rewrite poweRM ?inve_ge0 ?Lnorm_ge0 ?gt_eqF//.
  rewrite -[in LHS]poweR_inv1; last by apply fpos.
  congr (_ * _); rewrite -poweR_inv1; last first.
    rewrite poweR_gt0; repeat case: ifPn=>//=; last by move=> _; rewrite ltW.
    by move: (fpos); case: (ltgtP ('N_p%:E [f]) 0).
  rewrite poweRAC// /poweRrM_def; case:'N_p%:E [f] =>//=.  
  1,2: by rewrite ?lee_fin ?(ltW p0) ?orbT.
...
move: foo fp0; rewrite -!powR_Lnorm//.
case H : ('N_p%:E [f] `^ p%:E)=>//=; move=> _ r0.
under eq_integral do rewrite inver gt_eqF//.
by rewrite integralZl// -powR_Lnorm// H -EFinM mulVf// gt_eqF.
Qed.

Lemma ehoelder (f g : T -> \bar R) p q :
...
rewrite [leLHS](_ : _ = 'N_1[(F \* G)] * 'N_p%:E[f] * 'N_q%:E[g]); last first.
  rewrite !Lnorm1; under [in RHS]eq_integral.
    ...
    rewrite gee0_abs; last by rewrite !mule_ge0 ?inve_ge0 ?Lnorm_ge0//.
    by rewrite muleACA -abseM; over.
  rewrite ge0_integralZr//; last 2 first.
    - repeat apply: measurableT_comp=> //; exact: emeasurable_funM.
    - by rewrite mule_ge0// inve_ge0 Lnorm_ge0.
  rewrite -!muleA [X in _ * X](_ : _ = 1) ?mule1//; last first.
  rewrite [X in _ * X]muleC -muleA muleA [X in X * _]muleC. 
  by rewrite !divee ?ge0_fin_numE ?mule1 ?ltey ?Lnorm_ge0// gt_eqF.
rewrite -(mul1e ('N_p%:E[f] * _)) -muleA lee_pmul ?mule_ge0 ?Lnorm_ge0//.
rewrite [leRHS](_ : _ = \int[mu]_x (F x `^ p%:E / p%:E + G x `^ q%:E / q%:E)).
  rewrite Lnorm1 ae_ge0_le_integral//.
  - apply: measurableT_comp => //.
    by apply: emeasurable_funM; exact: measurable_normalized.
  - by move=> x _; rewrite adde_ge0 ?mule_ge0 ?poweR_ge0 ?inve_ge0// ltW. 
  - by apply /emeasurable_funD; apply /emeasurable_funM=> //;
    apply /measurableT_comp_poweR; exact /measurable_normalized.
  apply /aeW=> x _; rewrite gee0_abs ?conjugate_poweR ?normalized_ge0//.
  by rewrite mule_ge0// normalized_ge0.
rewrite ge0_integralD//; last 4 first.
  -by move=> x _; rewrite mule_ge0// ?inve_ge0 ?poweR_ge0// ltW.
  - apply: emeasurable_funM=> //.
    exact /measurableT_comp_poweR /measurable_normalized.
  - by move=> x _; rewrite mule_ge0// ?inve_ge0 ?poweR_ge0// ltW.
  - apply: emeasurable_funM=> //.
    exact /measurableT_comp_poweR /measurable_normalized.
...
- by rewrite inve_ge0 ltW.
rewrite ge0_integralZr//; last 3 first.
- exact /measurableT_comp_poweR /measurable_normalized.
- by move=> x _; rewrite poweR_ge0.
- by rewrite inve_ge0 ltW.
rewrite integral_normalized//; last exact: integrable_poweR.
rewrite integral_normalized//; last exact: integrable_poweR.
rewrite 2!mul1e /inve 2!(_: _ == 0%R = false) ?gt_eqF// -EFinD; f_equal.
by symmetry; apply pq.
Qed. 

Lemma hoelder (f g : T -> R) p q :
    measurable_fun [set: T] f -> measurable_fun [set: T] g ->
    (0 < p)%R -> (0 < q)%R -> (p^-1 + q^-1 = 1)%R ->
  'N_1[EFin \o (f \* g)%R] <= 'N_p%:E[EFin \o f] * 'N_q%:E[EFin \o g].
Proof.
move=> ? ? ? ?.
rewrite Lnorm1; under eq_integral do rewrite /= -abse_EFin EFinM.
by rewrite -Lnorm1; apply ehoelder=> //; apply: measurableT_comp.
Qed.
...
Lemma ehoelder2 (a1 a2 b1 b2 : \bar R) (p q : R) :
  0 <= a1 -> 0 <= a2 -> 0 <= b1 -> 0 <= b2 ->
  (0 < p)%R -> (0 < q)%R -> (p^-1 + q^-1 = 1)%R ->
  a1 * b1 + a2 * b2 <= (a1 `^ p%:E + a2 `^ p%:E) `^ p%:E^-1 *
                       (b1 `^ q%:E + b2 `^ q%:E) `^ q%:E^-1.
Proof.
...
have := ehoelder counting (mf a1 a2) (mf b1 b2) p0 q0 pq.
...
rewrite !poweRe1 ?lee_fin ?normr_ge0//.
rewrite (nneseries_split 0 2); last by move=> k; rewrite poweR_ge0.
rewrite ereal_series_cond eseries0 ?adde0; last first.
  by move=> [//|] [//|n _]; rewrite /f/= poweR_EFin normr0 powR0// gt_eqF.
rewrite big_mkord 2!big_ord_recr /= big_ord0 add0e. 
rewrite (nneseries_split 0 2); last by move=> k; rewrite poweR_ge0.
rewrite ereal_series_cond eseries0 ?adde0; last first.
  by move=> [//|] [//|n _]; rewrite /f/= poweR_EFin normr0 powR0// gt_eqF.
...
do 4 (rewrite gee0_abs //).
by rewrite /inve 2!(_: _ == 0%R = false) ?gt_eqF.
Qed.

Lemma hoelder2 (a1 a2 b1 b2 : R) (p q : R) :
  (0 <= a1)%R -> (0 <= a2)%R -> (0 <= b1)%R -> (0 <= b2)%R ->
  (0 < p)%R -> (0 < q)%R -> (p^-1 + q^-1 = 1)%R ->
  (a1 * b1 + a2 * b2 <= (a1 `^ p + a2 `^ p) `^ p^-1 *
                       (b1 `^ q + b2 `^ q) `^ q^-1)%R.
Proof.
move=> a10 a20 b10 b20 p0 q0 pq.
rewrite -lee_fin EFinD !EFinM -!poweR_EFin !EFinD -!poweR_EFin.
by rewrite -!inve_fin ?gt_eqF//; apply: ehoelder2.
Qed.
...
Context d (T : measurableType d) (R : realType).
Variable mu : {measure set T -> \bar R}.
Implicit Types (f g : T -> \bar R) (p : \bar R).

Local Open Scope ereal_scope.

Let convex_powR_abs_half f g (p : R) x : (1 <= p)%R ->
  `| 2^-1 * f x + 2^-1 * g x | `^ p%:E <=
   2^-1 * `| f x | `^ p%:E + 2^-1 * `| g x | `^ p%:E.
Proof.
move=> p1; rewrite (@le_trans _ _((2^-1 * `| f x | + 2^-1 * `| g x |)`^p%:E))//.
  rewrite ge0_lee_poweR ?lee_fin ?(le_trans _ p1) ?in_itv//=.
    - by rewrite abse_ge0 leey.
    - by rewrite leey adde_ge0 ?mule_ge0 ?inve_ge0.
  by rewrite (le_trans (lee_abs_add _ _))// 2!abseM gee0_abs ?inve_ge0.
set F := `|f x|; set G := `|g x|.
have : 0 <= F by rewrite abse_ge0.
have : 0 <= G by rewrite abse_ge0. 
have inv2N1 : (2^-1 = 1 -2^-1 :> R)%R by rewrite {2}(splitr 1) div1r addrK.
case: F=> [r||]; case: G=> [s||]// F0 G0; last first.
- rewrite gt0_poweRye ?lte_fin ?(lt_le_trans _ p1) ?gt0_muley ?inve_gt0//. 
  by rewrite addye// gt0_poweRye ?lte_fin ?(lt_le_trans _ p1).
- rewrite !gt0_poweRye ?lte_fin ?(lt_le_trans _ p1) ?gt0_muley ?inve_gt0//.
  rewrite !addye ?gt0_poweRye ?lte_fin ?(lt_le_trans _ p1)//.
    rewrite eq_sym -ltNye; apply /(@lt_le_trans _ _ 0)=> //.
    by rewrite mule_ge0 ?poweR_ge0 ?inve_ge0.
  rewrite eq_sym -ltNye; apply /(@lt_le_trans _ _ 0)=> //. 
  by rewrite mule_ge0 ?poweR_ge0 ?inve_ge0.
- rewrite gt0_poweRye ?lte_fin ?(lt_le_trans _ p1) ?gt0_muley ?inve_gt0//.
  rewrite !addey ?gt0_poweRye ?lte_fin ?(lt_le_trans _ p1)//.
    rewrite eq_sym -ltNye; apply /(@lt_le_trans _ _ 0)=> //.
    by rewrite mule_ge0 ?inve_ge0 ?poweR_ge0.
  rewrite eq_sym -ltNye; apply /(@lt_le_trans _ _ 0)=> //.
  by rewrite mule_ge0 ?inve_ge0 ?poweR_ge0.
- rewrite /inve !gt_eqF// -!EFinM -!EFinD poweR_EFin lee_fin.
  rewrite {2 4}inv2N1; apply: (convex_powR p1 (Itv01 _ _)) => //=.
  + by rewrite -subr_ge0 -inv2N1 invr_ge0.
  + by rewrite inE/= in_itv/= -lee_fin G0.
  + by rewrite inE/= in_itv/= -lee_fin F0.
Qed.
...
- by move=> x _; rewrite lee_abs_add.
Qed.
...
have gtp0 : (0 < p)%R by exact /(lt_le_trans _ p1).
have h x : (`| f x + g x | `^ p%:E <=
            2 `^ (p%:E - 1) * (`| f x | `^ p%:E + `| g x | `^ p%:E)).
  have := convex_powR_abs_half (fun x => 2 * f x) (fun x => 2 * g x) x p1.
  rewrite !abseM (@gee0_abs _ 2)// !muleA mulVe// !mul1e=> /le_trans; apply.
  move: p1; case: (ltgtP 1%R p)=>//= [p1 _|<- _]; last first.
    rewrite !poweRe1 ?mule_ge0// -EFinD subrr poweRe0. 
    by rewrite mul1e 2!muleA mulVe// !mul1e.
  rewrite !poweRM// !muleA -poweR_inv1// -poweRD; last first.
    rewrite poweRD_defE/= addrC -!EFinD !eqe subr_eq0; apply /implyP.
  by move: p1=> /[swap] /eqP -> /[!@ltxx].
  by rewrite (addeC _ p%:E) muleDr// ltninfty_adde_def// inE; 
    by apply /(lt_le_trans); last apply poweR_ge0.
rewrite unlock poweR_ltr//.
pose x := \int[mu]_x (2 `^ (p%:E - 1) * (`|f x| `^ p%:E + `|g x| `^ p%:E)).
set y := \int[mu]_x0  `|(f x0 + g x0)| `^ p%:E.
apply: (@le_lt_trans _ _ x).
  rewrite ge0_le_integral//=.
  - by move=> ? _; rewrite poweR_ge0.
  - apply/ measurableT_comp_poweR/measurableT_comp=>//.
    exact: emeasurable_funD.
  - apply/ emeasurable_funM/emeasurable_funD=> //;
    exact /measurableT_comp_poweR/measurableT_comp.
rewrite {}/x.
rewrite ge0_integralZl_EFin//; last first.
- by apply/emeasurable_funD; apply/measurableT_comp_poweR/measurableT_comp.
- by move=> ? _; rewrite adde_ge0 ?poweR_ge0.  
rewrite lte_mul_pinfty ?lee_fin ?powR_ge0//.
rewrite ge0_integralD//; last first.
- exact/measurableT_comp_poweR/measurableT_comp.
- by move=> ? _; rewrite poweR_ge0.
- exact/measurableT_comp_poweR/measurableT_comp.
- by move=> ? _; rewrite poweR_ge0.
rewrite lte_add_pinfty//.
  by rewrite -poweR_Lnorm// poweR_ltr//.
by rewrite -poweR_Lnorm// poweR_ltr//.
Qed.

Lemma eminkowski_EFin f g (p : R) :
  measurable_fun [set: T] f -> measurable_fun [set: T] g -> (1 <= p)%R ->
  'N_p%:E[(f \+ g)%R] <= 'N_p%:E[f] + 'N_p%:E[g].
Proof.
move=> mf mg; rewrite le_eqVlt=> /predU1P[<-|p1]; first exact:(minkowski1 p%:E).
...
under eq_integral.
  move=> x _.
  rewrite -mule_poweRB1; last by rewrite abse_ge0 !lte_fin p0 p1 implybT.
  over.
apply: (@le_trans _ _
    (\int[mu]_x ((`|f x| + `|g x|) * `|f x + g x| `^ (p - 1)%:E))).
  rewrite ge0_le_integral//.
  - by move=> ? _; rewrite mule_ge0 ?poweR_ge0.
  - apply /emeasurable_funM. 
      by apply /measurableT_comp=>//; apply/emeasurable_funD.
    by apply/measurableT_comp_poweR/measurableT_comp=>//;apply/emeasurable_funD.
  - apply /emeasurable_funM. 
      by apply /emeasurable_funD; apply /measurableT_comp.
  - by apply/measurableT_comp_poweR/measurableT_comp=>//;apply/emeasurable_funD.
  - by move=> ? _; rewrite -EFinD lee_wpmul2r ?poweR_ge0 ?lee_abs_add.
under eq_integral=> ? _ do rewrite ge0_muleDl ?abse_ge0//.
rewrite ge0_integralD//; last first.
- apply/emeasurable_funM.
    exact/measurableT_comp.
  by apply/measurableT_comp_poweR/measurableT_comp=>//;apply/emeasurable_funD.
...
rewrite [leRHS](_ : _ = ('N_p%:E[f] + 'N_p%:E[g]) *
  ...
  - rewrite fin_num_poweR -?poweR_Lnorm ?fin_num_poweR//.
    by rewrite ge0_fin_numE ?Lnorm_ge0.
  - by rewrite ge0_adde_def// inE Lnorm_ge0. 
  apply: leeD.
  - pose h := (@poweR R ^~ (p - 1)%:E \o abse \o (f \+ g)); pose i := (f \* h).
    rewrite [leLHS](_ : _ = 'N_1[i]%R); last first.
      rewrite Lnorm1; apply: eq_integral => x _ /=.
      by rewrite abseM (gee0_abs (poweR_ge0 _ _)).     
    rewrite [X in _ * X](_ : _ = 'N_(p / (p - 1))%:E[h]); last first.
      rewrite unlock.
      rewrite onemV ?gt_eqF// invf_div; apply: congr2; last by [].
      apply: eq_integral => x _.
      rewrite abse_poweR// abse_id -poweReM; last first.
        rewrite /poweRrM_def;case:`|_|=>//=. 
        by rewrite !lee_fin subr_ge0 [X in _ || X]ltW ?orbT.
      by rewrite -EFinM mulrC divfK ?gt_eqF ?subr_gt0.
    apply: (@ehoelder _ _ _ _ _ _ p (p / (p - 1))) => //.
    + apply/measurableT_comp_poweR/measurableT_comp=>//.
      exact/emeasurable_funD. 
    + by rewrite divr_gt0// subr_gt0.
    + by rewrite invf_div -onemV ?gt_eqF// subrKC.
  - pose h := (fun x => `|f x + g x| `^ (p - 1)%:E); pose i := (g \* h).
    rewrite [leLHS](_ : _ = 'N_1[i]); last first.
      rewrite Lnorm1; apply: eq_integral => x _ /=.
      by rewrite abseM abse_poweR// abse_id.
    rewrite [X in _ * X](_ : _ = 'N_((1 - p^-1)^-1)%:E[h])//; last first.
      rewrite unlock.
      apply: congr2; last by rewrite invrK.
      apply eq_integral=> ? _; rewrite -/(onem p^-1) onemV ?gt_eqF// /h.
      rewrite abse_poweR// abse_id -poweReM; last first.
        rewrite /poweRrM_def; case: `|_|=>//; rewrite !lee_fin subr_ge0.
        by rewrite [X in _ || X]ltW ?orbT.
      by rewrite -EFinM divKf// subr_eq0 gt_eqF.
    apply: (le_trans (@ehoelder _ _ _ _ _ _ p (1 - p^-1)^-1 _ _ _ _ _)) => //.
    + apply/measurableT_comp_poweR/measurableT_comp=>//.
      exact/emeasurable_funD. 
    + by rewrite invr_gt0 onem_gt0// invf_lt1.
    + by rewrite invrK subrKC.
rewrite -muleA; congr (_ * _).
under [X in X * _]eq_integral=> x _.
  rewrite mule_poweRB1; last by rewrite abse_ge0 !lte_fin p0 p1 implybT.
over.
rewrite EFinD poweRD; last first.
  rewrite -powR_Lnorm//.
  move: (Nfgoo).
  set Nfg := 'N_p%:E [(f \+ g)%R].
  have: 0 <= Nfg by rewrite Lnorm_ge0.
  case: Nfg=>//= r r0 _.
  by rewrite subr_eq0 eq_sym invr_eq1 gt_eqF.
rewrite poweRe1; last by rewrite integral_ge0// => ? _; rewrite poweR_ge0.
congr (_ * _).
rewrite (_ : (- p^-1)%:E = - (p^-1)%:E)// poweRN//=; last first.
- by rewrite -powR_Lnorm// fin_num_poweR// ge0_fin_numE ?Lnorm_ge0//.
- by apply: integral_ge0=> ? _; rewrite poweR_ge0.
- rewrite -powR_Lnorm// fine_poweR ?Lnorm_ge0//=.
  by rewrite -powRrM divff ?gt_eqF// powRr1//= fine_ge0// Lnorm_ge0//.
Qed.

Lemma lerB_DLnorm f g (p : R) :
  measurable_fun [set: T] f -> measurable_fun [set: T] g -> (1 <= p)%R ->
  'N_p%:E[f] <= 'N_p%:E[f \+ g] + 'N_p%:E[g].
Proof.
move=> mf mg p1.
apply /(@le_trans _ _ ('N_p%:E[((f + g) + (-%E \o g))])).
  rewrite -Lnorm_abse -[X in _ <= X]Lnorm_abse le_Lnorm ?(le_trans ler01)//.
  - exact/measurableT_comp.
  - apply/measurableT_comp=>//;apply/emeasurable_funD.
      exact/emeasurable_funD.
    exact/measurableT_comp.
  - move=> x _//=; rewrite !abse_id compE. 
    rewrite (_: (f + g + \- g) x = (f x)+(g x)-(g x))//=;set gx := g x.
    by case: (gx)=> [r||]; rewrite ?addeNy/= ?leey//= -addeA subee// adde0.
rewrite (_ : 'N__[g] = 'N_p%:E[-%E \o g]); last by apply/esym/oppe_Lnorm.
by apply: eminkowski_EFin=>//;[exact/emeasurable_funD|exact/measurableT_comp].
Qed.

Lemma lerB_LnormD f g (p : R) :
  measurable_fun [set: T] f -> measurable_fun [set: T] g -> (1 <= p)%R ->
...
Proof.
case: p => //[r|]; first exact: eminkowski_EFin.
move=> mf mg _; rewrite unlock /Lnorm.
case: ifPn => mugt0; last by rewrite adde0 lexx.
apply /ess_sup_absD.
Qed.

Lemma minkowski_EFin (f g: T -> R) (p : R) :
  measurable_fun [set: T] f -> measurable_fun [set: T] g -> (1 <= p)%R ->
  'N_p%:E[EFin \o (f \+ g)%R] <= 'N_p%:E[EFin \o f] + 'N_p%:E[EFin \o g].
Proof.
move=> mf mg p1 //=.
rewrite Lnorm_EFin; under eq_integral do rewrite compE/= -abse_EFin EFinD.
by rewrite -Lnorm_EFin; apply/eminkowski=>//=;exact/measurableT_comp.
Qed.

Lemma minkowski (f g: T -> R) p :
  measurable_fun [set: T] f -> measurable_fun [set: T] g -> (1 <= p) ->
  'N_p[EFin \o (f \+ g)%R] <= 'N_p[EFin \o f] + 'N_p[EFin \o g].
...
rewrite (le_lt_trans (@minkowski _ _ _ mu f g p _ _ _))//.
...
apply: mem_set => /=; apply: (le_lt_trans (minkowski _ _ _ _)) => //.
...
Proof. by rewrite -lee_fin EFinD !finite_norm_fine minkowski. Qed.
...
rewrite (@lty_poweRy _ _ 2^-1) ?inve_gt0//.
...
rewrite poweR_ltr//=.
under eq_integral => x _ do rewrite normrM poweR_EFin powRM// EFinM.
...
rewrite /finite_norm unlock poweR_ltr//.
under eq_integral => x _/= do 
  rewrite poweR_EFin (_ : `|c| `^ r = cst (`|c| `^ r) x)//.
...
rewrite le_eqVlt => /predU1P[mu0 p1 q1 muTfin pq f +|mu_pos].
  ...
  under eq_integral do rewrite /= poweR_EFin -[(_ `^ _)%R]ger0_norm ?powR_ge0//=.
  rewrite (@integral_abs_eq0 _ _ _ _ _ (fun x => (`|f x| `^ r)%:E))//.
    by rewrite poweR_EFin ltry.
...
- move=> mu_fin.
  ...
  apply/poweR_ltr/(le_lt_trans h1).
...
- have p0 : (0 < p)%R by rewrite ?(lt_le_trans ltr01).
  ...
  rewrite poweR_ltr//; move: supf_lty => /ess_supr_bounded[M fM].
...
rewrite ?(gt_eqF p0)// (poweRe1 (ltW fp0))//.
...
set y := \int[mu]_x0  `|(f x0 + g x0)%E|%:E `^ p%:E.
apply: (@le_lt_trans _ _ x).
  rewrite ge0_le_integral//=.
...
rewrite ge0_integralD//; last 2 first.
  ...
  - exact/measurable_EFinP/measurableT_comp_powR/measurableT_comp.
rewrite lte_add_pinfty//; under eq_integral do rewrite -poweR_EFin -abse_EFin.
  by rewrite -powR_Lnorm// poweR_ltr//=; case: ifPn; rewrite ?ltW// Nfoo ltry.
by rewrite -powR_Lnorm// poweR_ltr//=; case: ifPn; rewrite ?ltW// Ngoo ltry.
Qed.
...
rewrite EFinD poweRD; last first.
  ...
  case: Nfg=>//= r r0 _.
  by rewrite subr_eq0 eq_sym invr_eq1 gt_eqF.
rewrite poweRe1; last by rewrite integral_ge0.
congr (_ * _).
rewrite (_ : (- p^-1)%:E = - (p^-1)%:E)// poweRN//=; last first.
- under eq_integral do rewrite -poweR_EFin -abse_EFin.
  by rewrite -powR_Lnorm// fin_num_poweR// ge0_fin_numE ?Lnorm_ge0//.
- exact: integral_ge0 => x _; rewrite lee_fin powR_ge0.
under eq_integral do rewrite -poweR_EFin -abse_EFin.
  rewrite -powR_Lnorm// fine_poweR ?Lnorm_ge0//=.
  by rewrite -powRrM divff ?gt_eqF// powRr1//= fine_ge0// Lnorm_ge0//.
Qed.
...
(*
Section hoelder_ereal.
Context d {T : measurableType d} {R : realType}.
Variable mu : {measure set T -> \bar R}.
Local Open Scope ereal_scope.
Implicit Types (p q : R) (y z : \bar R) (f g : T -> \bar R).

Let measurableT_comp_poweR f y :
  measurable_fun [set: T] f -> measurable_fun setT (fun x => f x `^ y).
Proof. 
exact /(@measurableT_comp _ _ _ _ _ _ (@poweR R ^~ y)) /measurable_poweR. Qed.
...
Let integrable_poweR f p : (0 < p)%R ->
    measurable_fun [set: T] f -> 'N_p%:E[f] != +oo ->
  mu.-integrable [set: T] (fun x => (`|f x| `^ p%:E)).
Proof.
move=> p0 mf foo; apply/integrableP; split.
...
Lemma hoelder_ereal (f g : T -> \bar R) p q :
    measurable_fun [set: T] f -> measurable_fun [set: T] g ->
    (0 < p)%R -> (0 < q)%R -> (p^-1 + q^-1 = 1)%R ->
  'N_1[(f \* g)] <= 'N_p%:E[f] * 'N_q%:E[g].
Proof.
